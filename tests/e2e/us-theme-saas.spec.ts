...
test('미국 테마 흐름은 ETF를 뺀 TOP50과 5개 고정 테마 및 정규장/애프터 분리 화면을 표시한다', async ({ page }, testInfo) => {
  ...
  const rankingRail = page.getByTestId('us-top100-ranking')
  await expect(rankingRail).toBeVisible()
  await expect(rankingRail.getByText('순위·거래대금·정규장 등락률은 16:00 ET 스냅샷 고정 · 애프터 등락률만 갱신')).toBeVisible()
  await expect(rankingRail.getByText('정규장', { exact: true })).toBeVisible()
  await expect(rankingRail.getByText('애프터', { exact: true })).toBeVisible()
