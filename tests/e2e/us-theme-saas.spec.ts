...
test('미국 테마 흐름은 ETF를 뺀 TOP50과 5개 고정 테마 및 정규장·애프터 분리를 표시한다', async ({ page }, testInfo) => {
  await page.goto('/')
  ...
  await expect(page.getByTestId('us-after-hours-movers')).toBeVisible()
  ...
  await page.screenshot({ path: testInfo.outputPath('us-theme-top50-five-session-split-dashboard.png'), fullPage: true })
})
