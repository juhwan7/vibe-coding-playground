  ...
  return <div className="app-root">
    <nav className="global-nav">
      ...
      <aside className="nav-actions">
        ...
        <button className={`manual-refresh ${refreshFailed ? 'failed' : ''}`} type="button" onClick={manualRefresh} disabled={refreshing} aria-label="시장 데이터 수동 새로고침" title="첫 화면 핵심 데이터를 우선 갱신합니다. 서버에서 20초 쿨다운을 적용합니다.">{refreshing ? '갱신 중…' : refreshFailed ? '잠시 후 재시도' : '↻ 새로고침'}</button>
