...
export default function StockQuiz() {
  ...
  const start = (nextPool: QuizPool) => {
    const selectedPool = pools[nextPool].filter((stock) => Boolean(stock.description)).slice(0, poolLimit(nextPool))
  ...
  if (!pool) {
    const kospiReady = pools.kospi200.filter((stock) => Boolean(stock.description)).length
    const kosdaqReady = pools.kosdaq150.filter((stock) => Boolean(stock.description)).length
    ...
    return <main className="index-quiz-shell" data-testid="index-quiz">
      <section className="index-quiz-select">
        ...
        <p className="index-quiz-lead">Raspberry Pi가 종목코드·종목명·시장·기업설명을 하나의 완성 캐시로 미리 저장합니다. 메뉴와 문제 이동에서는 외부 사이트를 조회하지 않고 준비된 JSON만 사용합니다.</p>
        ...
        <div className="index-quiz-source"><b>ETF·ETN 제외</b><span>{loading && !prepared ? 'Pi 퀴즈 캐시 읽는 중' : prepared?.source ?? 'Pi 퀴즈 캐시'}{prepared?.sourceDate ? ` · 기준 ${prepared.sourceDate}` : ''}{prepared?.stale ? ' · 저장된 최근 구성종목 사용 중' : ''}</span></div>
        {loadError && !prepared && <div className="index-description-state error"><strong>Pi 퀴즈 캐시를 읽지 못했습니다.</strong><span>시장 데이터와 별개로 퀴즈 준비본 연결만 다시 확인하면 됩니다.</span><button onClick={() => { setLoading(true); void warmQuizPrepared(true).then((payload) => { if (payload) { setPrepared(payload); setLoadError(false) } setLoading(false) }) }}>다시 확인</button></div>}
      </section>
    </main>
  }
  ...
  if (!question) return <main className="index-quiz-shell"><div className="index-quiz-loading">출제 가능한 Pi 캐시를 확인하고 있습니다.</div></main>
