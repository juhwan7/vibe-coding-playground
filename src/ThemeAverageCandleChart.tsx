...
export default function ThemeAverageCandleChart({ theme, accent }: { theme: ThemeGroup; accent: string }) {
  ...
  if (!points.length) return <div className="theme-chart-empty">테마 평균 3분 선차트 데이터 수집 중</div>
  ...
  const minVisibleRange = Math.max(0.04, Math.abs(center) * 0.006)
  const visibleRange = Math.max(rawRange, minVisibleRange)
  const pad = visibleRange * 0.10
  ...
  return <div className="theme-chart-wrap" style={{ ['--theme-accent' as string]: accent }}>
    <div className="theme-chart-title">
      <span className="theme-chart-name">테마 평균 3분 선차트 <small>자동 확대축 · 실제 평균 등락률</small></span>
      <div className="theme-chart-metrics">
        <span>구간 고점 <b>{fmtRate(rawMax)}</b></span>
        <span>구간 저점 <b>{fmtRate(rawMin)}</b></span>
      </div>
    </div>
    <svg className="theme-chart" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" aria-label={`${theme.name} 구성종목 평균 3분 선차트`}>
      ...
      {ticks.map((tick) => {
        ...
        return <g key={`${tick.day}-${tick.hour}`}>
          ...
          <text x={Math.min(width - 34, x + 3)} y="182" className="theme-hour-label">{String(tick.hour).padStart(2, '0')}:00</text>
        </g>
      })}
      ...
      <circle cx={xForTimestamp(latest.timestamp, latest.day)} cy={y(latest.lineValue)} r="3.2" className="theme-average-current-dot">
